...
$repoURL = "https://$githubKey@github.com/pierresh/frankenphp.git";

// Constants
const DOCS_TO_CLONE = "docs/cn";
const DESTINATION_DIRECTORY = __DIR__ . "/content/cn";
const DOCS_DESTINATION = DESTINATION_DIRECTORY . "/docs";
const NAV_DESTINATION = __DIR__ . "/data/cn/nav.yaml";
const TEMP_DIR = __DIR__ . "/temp-documentation-cn";
...
function fixLinks($content)
{
    ...
    $content = preg_replace_callback(
        '/(?<=^|[^a-zA-Z0-9])\/docs\/(?!cn)([^\/]+)\/?/',
        function ($matches) {
            // Ajoute '/docs/cn/' au début et s'assure qu'il y a un slash à la fin.
            return '/docs/cn/' . $matches[1] . '/';
        },
        $content
    );
