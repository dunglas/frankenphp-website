...
如果从 `$PHP_INI_DIR/php.ini-production` 或 `$PHP_INI_DIR/php.ini-development` 复制 `php.ini`，您还必须设置变量 `variables_order = "EGPCS"`，因为 `variables_order` 的默认值是 `"EGPCS"`，但在 `php.ini-production` 和 `php.ini-development` 中，我们有 `"GPCS"`。在这种情况下，	`worker` 无法正常工作。
...
或者，可以在全局选项下指定要创建的线程数和要从服务器启动的 [worker scripts](/cn/docs/worker/)。
